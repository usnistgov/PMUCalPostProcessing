            ...
            while (i <= numel(self.dataFiles))
                ...
                switch iFactor
                    case {'KxS','KaS'}
                        ...
                        % Write the header to the active sheet
                        hdr = string(C(1,:));
        ...
        function plotExcelAnalysis(self)
           ...
           for i = nSheets:-1:1
              ...
              switch sheetName
                  ...
                  case {'phase step','amplitude step'}
                      % setp test plots
                      [respTime, fig] = self.plotStepVoltageResponseTime(T,Lim(1),xLabel(1));
                      self.hExcel.WriteData(fig,'B4')
                      
                      % convert respTime to a table                      
                      self.hExcel.WriteData(respTime,'B1')
                      close(fig)
                      
                      % setp test plots
                      [respTime, fig] = self.plotStepCurrentResponseTime(T,Lim(1),xLabel(1));
                      self.hExcel.WriteData(fig,'J4')
                      
                      % convert respTime to a table                      
                      self.hExcel.WriteData(respTime,'J1')
                      close(fig)
